...
if ($group_id) {
	...
	for ($j = 0; $j < $mlCount; $j++) {
		...
		if ($currentList->isError()) {
			...
			echo '<td width="50%" colspan="2"><strong>'.$currentList->getName().'</strong></td>'.
				 '<td width="25%">'.htmlspecialchars($currentList->getDescription()). '</td>'.
				 '<td width="25%" style="text-align:center">'._('Not activated yet').'</td>';
		} else {
			...
			echo '<td width="25%">'.
				...
				'<td width="25%">'.htmlspecialchars($currentList->getDescription()). '</td>'.
				'<td width="25%" style="text-align:center"><a href="'.$currentList->getExternalInfoUrl().'">'._('Subscribe/Unsubscribe/Preferences').'</a>'.
				'</td>';
		}
		echo '</tr>';
	}
