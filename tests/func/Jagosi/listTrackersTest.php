...
class ListTrackersJava extends FForge_SeleniumTestCase
{
	function testListTrackersJava()
	{
		$this->init();
		...
		// Use Jagosi to get the list of bugs in given tracker
		$this->Jagosi("DumpTracker.sh -u admin -p myadmin -g projecta -t Bugs", " | grep 'Summary' | wc -l");
		unlink('Bugs.csv');
		$number = $this->getOutput();
		$this->assertEquals(10, $number[0]);

		// Use Jagosi to get the list of bugs in given tracker
		$this->Jagosi("DumpTracker.sh -u admin -p myadmin -g projecta -t 'Feature Requests'", " | grep 'Summary' | wc -l");
		unlink('Feature Requests.csv');
		$number = $this->getOutput();
