...
class WikiACL extends FForge_SeleniumTestCase
{
	function testwikiACL()
	{
		...
		// TEST01:
		// Follow the wiki ACL step by step page to give write access to the HomePage
		// to users that are not member of the project.
		$this->clickAndWait("link=Home Page");
		$this->clickAndWait("link=Access Rights");
		$this->clickAndWait("link=SetAcl");

		$this->click("acl[_add_group][edit]");
		$this->click("acl[_new_perm][edit]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->assertTextPresent("ACL changed for page 'HomePage'");
		$this->assertTextPresent("from 'None'");
		$this->assertTextPresent("to 'view:_EVERY; edit:_AUTHENTICATED,-_BOGOUSER,_EVERY; create:_AUTHENTICATED,-_BOGOUSER; list:_EVERY; remove:_ADMIN,_OWNER; purge:_ADMIN,_OWNER; dump:_ADMIN; change:_ADMIN,_OWNER'.");
		
		// TEST02:
		// Follow the Wiki ACL step by step page to give read
		// access only to project members.
		// Page is: "MySecret"
		$this->clickAndWait("link=Home Page");
		...
		// Step1: Add a rule with view access to Authenticated Users
		$this->clickAndWait("link=Home Page");
		$this->clickAndWait("link=MySecret");
		$this->clickAndWait("link=Access Rights");
		$this->clickAndWait("link=SetAcl");

		$this->select("acl[_new_group][view]", "label=Authenticated Users");
		$this->click("acl[_new_perm][view]");
		$this->click("acl[_add_group][view]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->assertTextPresent("ACL changed for page 'MySecret'");
		$this->assertTextPresent("to 'view:_EVERY,_AUTHENTICATED; edit:_AUTHENTICATED,-_BOGOUSER; create:_AUTHENTICATED,-_BOGOUSER; list:_EVERY; remove:_ADMIN,_OWNER; purge:_ADMIN,_OWNER; dump:_ADMIN; change:_ADMIN,_OWNER'.");

		// Step2: remove view access rights for Every.
		$this->clickAndWait("link=MySecret");
		$this->clickAndWait("link=Access Rights");
		$this->clickAndWait("link=SetAcl");

		$this->click("acl[_del_group][view][_EVERY]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->assertTextPresent("ACL changed for page 'MySecret'");
		$this->assertTextPresent("to 'view:_AUTHENTICATED; edit:_AUTHENTICATED,-_BOGOUSER; create:_AUTHENTICATED,-_BOGOUSER; list:_EVERY; remove:_ADMIN,_OWNER; purge:_ADMIN,_OWNER; dump:_ADMIN; change:_ADMIN,_OWNER'.");

		// Seems to be a bug here, the next step is not sucessful
		// if it is executed right after the previous one.
		// So, return to the page and setacl.
		$this->clickAndWait("link=Home Page");
		$this->clickAndWait("link=MySecret");
		$this->clickAndWait("link=Access Rights");
		$this->clickAndWait("link=SetAcl");

		// Step3: add a forbid view access rule for Every.
		$this->click("acl[_add_group][view]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->clickAndWait("admin_setacl[acl]");
		$this->assertTextPresent("ACL changed for page 'MySecret'");
		$this->assertTextPresent("to 'view:_AUTHENTICATED,-_EVERY; edit:_AUTHENTICATED,-_BOGOUSER; create:_AUTHENTICATED,-_BOGOUSER; list:_EVERY; remove:_ADMIN,_OWNER; purge:_ADMIN,_OWNER; dump:_ADMIN; change:_ADMIN,_OWNER'.");
