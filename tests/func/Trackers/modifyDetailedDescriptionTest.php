...
class ModifyDetailedDescription extends FForge_SeleniumTestCase
{
	function testModifyDetailedDescription()
	{
		$this->init();
		...
		$this->click("//img[@alt='Click to edit']");
		$this->type("description", 'NEW detailed \n description with URL: http://www.fusionforge.org');
		$this->clickAndWait("submit");
		$this->assertTextPresent("regexp:Item \[#[0-9]+\] successfully updated");
		$this->clickAndWait("link=MyTicketSummary");
		$this->assertTextPresent('NEW detailed \n description with URL');
		$this->assertEquals("http://www.fusionforge.org", $this->getText("link=exact:http://www.fusionforge.org"));
